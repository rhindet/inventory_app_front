...
class InventoryViewModel(private val itemDao: ItemDao) : ViewModel() {
    ...
    private suspend fun fetchItemsFromApi(): List<Item> {
        return withContext(Dispatchers.IO) {
            try {
                val url = URL("http://192.168.0.16:3000/products")
    ...
    private fun parseItems(response: String): List<Item> {
        ...
        for (i in 0 until jsonArray.length()) {
            ...
            val item = Item(
                ...
                itemPrice = jsonObject.getDouble("price"),
                quantityInStock = jsonObject.getInt("quantity")
            )
            items.add(item)
        }
        return items
